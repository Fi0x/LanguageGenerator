...
public class TestFileController
{
	...
	{
		mvc.perform(get(UPLOAD_URL)).andExpect(status().isOk()).andExpect(forwardedUrl("/WEB-INF/jsp/upload.jsp"));
	}
	...
	void test_uploadLanguage_fileError() throws Exception
	{
		...
		mvc.perform(MockMvcRequestBuilders.multipart(UPLOAD_URL).file(file))
				.andExpect(status().is(HttpStatus.BAD_REQUEST.value())).andExpect(forwardedUrl(null));
	}
	...
	{
		MockMultipartFile file = new MockMultipartFile("languageFile", "filename.txt", "application/json",
				"invalid content".getBytes());

		mvc.perform(MockMvcRequestBuilders.multipart(UPLOAD_URL).file(file))
				.andExpect(status().is(HttpStatus.INTERNAL_SERVER_ERROR.value())).andExpect(forwardedUrl(null));
	}
	...
	void test_downloadLanguageFile_success() throws Exception
	{
		...
		mvc.perform(get(DOWNLOAD_URL).param("languageId", String.valueOf(LANGUAGE_ID)))
				.andExpect(status().is(HttpStatus.OK.value())).andExpect(
						header().string(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + LANGUAGE_NAME +
								".json\""))
