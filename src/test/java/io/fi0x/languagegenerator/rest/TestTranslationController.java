...
public class TestTranslationController
{
	...
	void test_showWord_success() throws Exception
	{
		...
		mvc.perform(get(WORD_URL).param("languageId", String.valueOf(LANGUAGE_ID)).param("word", WORD))
				.andExpect(model().attributeExists("word")).andExpect(model().attributeExists("username"))
				.andExpect(model().attributeExists("originalLanguageData"))
				.andExpect(model().attributeExists("translations")).andExpect(status().is(HttpStatus.OK.value()))
	...
	void test_showWord_unauthorized() throws Exception
	{
		...
		mvc.perform(get(WORD_URL).param("languageId", String.valueOf(LANGUAGE_ID)).param("word", WORD))
				.andExpect(status().is(HttpStatus.FORBIDDEN.value())).andExpect(forwardedUrl(null));
	}
	...
	void test_showDictionary_success() throws Exception
	{
		...
		doReturn(LanguageData.builder().id(LANGUAGE_ID).name(LANGUAGE_NAME).username(USERNAME).build()).when(
				languageService).getAuthenticatedLanguageData(eq(LANGUAGE_ID));
	...
	void test_saveTranslation_success() throws Exception
	{
		...
		mvc.perform(post(TRANSLATION_URL).param("languageId", String.valueOf(LANGUAGE_ID)).param("word", WORD)
						.param("translationLanguageId", String.valueOf(TRANSLATED_LANGUAGE_ID))
						.param("translationWord", TRANSLATED_WORD).sessionAttr("translations", getTranslationsList()))
				.andExpect(status().is(HttpStatus.OK.value())).andExpect(model().attributeExists("translations"))
				.andExpect(forwardedUrl("/WEB-INF/jsp/word.jsp"));
	}
	...
	void test_deleteTranslation_success() throws Exception
	{
		doNothing().when(translationService)
				.deleteTranslation(eq(LANGUAGE_ID), eq(WORD_NUMBER), eq(TRANSLATED_LANGUAGE_ID),
						eq(TRANSLATED_WORD_NUMBER));

		mvc.perform(get(DELETE_TRANSLATION_URL).param("languageId1", String.valueOf(LANGUAGE_ID))
						.param("wordNumber1", String.valueOf(WORD_NUMBER))
						.param("languageId2", String.valueOf(TRANSLATED_LANGUAGE_ID))
						.param("wordNumber2", String.valueOf(TRANSLATED_WORD_NUMBER)))
				.andExpect(status().is(HttpStatus.FOUND.value())).andExpect(redirectedUrl("/"));
	}
	...
	{
		doThrow(IllegalAccessException.class).when(translationService)
				.deleteTranslation(eq(LANGUAGE_ID), eq(WORD_NUMBER), eq(TRANSLATED_LANGUAGE_ID),
						eq(TRANSLATED_WORD_NUMBER));
		...
		mvc.perform(get(DELETE_TRANSLATION_URL).param("languageId1", String.valueOf(LANGUAGE_ID))
						...
						.param("languageId2", String.valueOf(TRANSLATED_LANGUAGE_ID))
						.param("wordNumber2", String.valueOf(TRANSLATED_WORD_NUMBER)))
				.andExpect(status().is(HttpStatus.FORBIDDEN.value())).andExpect(redirectedUrl(null));
	}
